document.addEventListener("DOMContentLoaded", function (event) {
    ...
    let makeTOC = function (contentElement, tocElement) {
        let formerTOC = document.querySelectorAll(".aqtoc");
        if (formerTOC) {
            for (let index = 0; index < formerTOC.length; index++) {
                const element = formerTOC[index];
                element.parentNode.removeChild(element);
            }
        }

        let htags = contentElement.querySelectorAll('h1, h2');
        ...
        for (let index = 0; index < htags.length; index++) {
            ...
            } else if (current.innerText === "Script Editor") {
        ...
        tocElement.insertBefore(tocDiv, tocElement.firstChild);
        document.body.insertBefore(styleEl, document.body.firstChild);

    };


    makeTOC(document.querySelector(".ms-rte-layoutszone-outer"), document.getElementById("contentBox"));
